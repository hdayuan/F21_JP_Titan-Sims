...
python3 Titan-tau-incl-con.py [num samples] [initial a] [target final a] [div factor]
[initial eccentricity] [initial inclination] [initial time]
...
"""
def main(numSamples, ia_titanRS, fa_titanRS, file, factor, ie_titan, ii_titan, iTime):
    ...
    # add Titan
    sim.add(m=mTitan, a=ia_titanAU, e=ie_titan, inc=ii_titan, hash = "Titan")
...
ie_titan = float(sys.argv[5])
ii_titan = float(sys.argv[6])
iTime = float(sys.argv[7])


# open file
f = open("v3out-div-con"+str(div_factor)+"-"+str(numSamples)+"s-"+str(ia_titan)+"to"+str(fa_titan)+"rs.txt", "a")
...
# call main
integTime = main(numSamples, ia_titan, fa_titan, f, div_factor, ie_titan, ii_titan, iTime)
