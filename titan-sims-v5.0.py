...
Description:
Integrates the Titan-Saturn-Sun-Jupiter system as Titan migrates
through the evection resonance. Includes Saturn's obliquity {and the tidal
effects on Saturn and Titan, but not on the Sun or Jupiter}. Includes
Jupiter to cause oscillations in Saturn's eccentricity. 
Uses the JANUS integrator in REBOUND because it is the fastest that gives
accurate results for Titan's eccentricity.

To run:

python3 titan-sims-v5.py [continuation?] [initial a] [number of samples]
...
def cont_sim_file(iaTitanRS, numSamples, intTime, numSamplesADD, intTimeADD):
    ...
    f_prev = open(r"v5.0-"+file_prev_str+".txt", "r")
    f_new = open("v5.0-"+file_new_str+".txt", "a")
    ...
    f_new.write("Continuation of: v5.0-"+file_prev_str+".txt\n")
...
def continue_sim(iaTitanRS, numSamples, intTime, numSamplesADD, intTimeADD, file):
    ...
    rSat = 58232503./AU_TO_M # mean physical radius of Saturn
    oSat = 26.7 * np.pi / 180. # obliquity of Saturn

    file_prev_str = str(iaTitanRS)+"rs-"+str(numSamples)+"s-"+str(intTime)+"myrs"
    file_new_str = str(iaTitanRS)+"rs-"+str(numSamples+numSamplesADD)+"s-"+str(intTime+intTimeADD)+"myrs"
    
    # reload previous simulation
    sim = rebound.Simulation("v5.0-sim-"+file_prev_str+".bin")
    rebx = reboundx.Extras(sim, "v5.0-simx-"+file_prev_str+".bin")
    ps = sim.particles
    ...
    for i in range(1, numSamplesADD + 1):
        ...
        sim.move_to_hel()
        # write output
        orbTitan = ps['Titan'].calculate_orbit(primary=ps['Saturn'])
        orbSat = ps['Saturn'].calculate_orbit(primary=ps['Sun'])
        file.write(str(orbTitan.a/rSat)+"\t"+str(orbTitan.e)+"\t"+str(orbTitan.inc-oSat)+"\t")
        file.write(str(orbTitan.pomega)+"\t"+str(orbSat.l)+"\t"+str(orbSat.e)+"\t")
        file.write(str(sim.t)+"\n")

    # remove old binary files of saved simulation
    os.remove("v5.0-sim-"+file_prev_str+".bin")
    os.remove("v5.0-simx-"+file_prev_str+".bin")

    # save simulation
    sim.save("v5.0-sim-"+file_new_str+".bin")
    rebx.save("v5.0-simx-"+file_new_str+".bin")
...
'mean longitude of Saturn'[tab]'e of Saturn'[tab]'current time in simulation'
...
def integrate_sim(iaTitanRS, numSamples, intTime, file):
    ...
    # add Sun, Jupiter, Saturn, and Titan 
    ...
    sim.add(rebound.Particle(sim, primary=sim.particles['Saturn'], m=mTitan, a=iaTitan, e=eTitan, inc=oSat, hash='Titan'))
    ...
    # save simulation
    file_str = str(iaTitanRS)+"rs-"+str(numSamples)+"s-"+str(intTime)+"myrs"
    sim.save("v5.0-sim-"+file_str+".bin")
    rebx.save("v5.0-simx-"+file_str+".bin")
...
def main():
    ...
    if (continuation == 1):
        file_str = str(iaTitanRS)+"rs-"+str(numSamples)+"s-"+str(intTime)+"myrs"
        if (not os.path.exists("v5.0-sim-"+file_str+".bin")) or (not os.path.exists("v5.0-simx-"+file_str+".bin")):
            raise Exception("One or more binary files for previous simulation to be continued does not exist")
        if not os.path.exists("v5.0-"+file_str+".txt"):
